...
| kev-9b | gpqaDiamond, sweBench, sweBenchPro, terminalBench, hle, lmarenaElo, gdpvalAA, arcAgi2, mmluPro, aime | 2026-09-23 | not applicable, not merely unpublished: Kev-9B is a typed-decision model (LoRA adapter + pointer head on Qwen3.5-9B-Base) that cannot generate text, so no generative benchmark can be run on it; the repo's frozen suites and the third-party sets it reruns (scienthoon, SemIf, WANLI) report classification accuracy, Brier and calibration only. The README's MMLU 0.74 / MMLU-Pro 0.52 are 10-way typed-choice accuracies on its own selections, not the benchmark protocol, so mmluPro stays null |
| kev-9b | maxOutput | 2026-09-23 | no output-token limit exists; output is a fixed set of typed values with probabilities, not a token stream (usage.output_tokens counts serialized answers) |
| kev-9b | pricing | 2026-09-23 | no hosted endpoint anywhere: OpenRouter and impossibl model lists checked 2026-09-23 (both list Jev and Laya, neither lists Kev), the Hugging Face Space is a free demo of Kev-4B/0.8B only, HelixML documents self-hosting only |
| kev-9b | knowledgeCutoff | 2026-09-23 | not disclosed on the Hugging Face card, GitHub README, release notes or PLAN.md; the Qwen3.5-9B-Base pretraining cutoff is not a published cutoff for Kev |
| kev-9b | costPerTask | 2026-09-23 | no Artificial Analysis coverage of Kev or any Jared Palmer model as of launch |
