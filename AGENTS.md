...
- **models.json** — `{ id, name, company (companies.json id), releaseDate "YYYY-MM-DD", status: frontier|superseded|unknown|deprecated, tier: flagship|balanced|fast, modality: text|multimodal, contextWindow, maxOutput, pricing: {inputPerMTok, outputPerMTok}, costPerTask: {usd, effort}, openWeights, knowledgeCutoff, benchmarks: {mmluPro, gpqaDiamond, sweBench, terminalBench, aime, hle, lmarenaElo, arcAgi2}, strengths[], weaknesses[], notes }`. Numbers are launch-time reported scores; null = unpublished/unverified. `costPerTask` is Artificial Analysis' cost per Intelligence Index task (USD), taken at medium effort wherever AA publishes one — it's the headline cost figure on the model card, while per-token `pricing` moved to the drawer. `status` is computed by `scripts/frontier-status.js` (see `protocols/FRONTIER_STATUS_PROTOCOL.md`), not hand-assigned — except `deprecated`, which stays manual.
