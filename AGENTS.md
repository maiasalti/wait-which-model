...
- **Cooldown:** a merge within 20 hours of the last email (`NOTIFY_COOLDOWN_HOURS`) queues its models in the `NOTIFY_PENDING_IDS` repository variable instead of sending; the workflow's daily 09:00 UTC scheduled run flushes the queue as one digest. `NOTIFY_LAST_SENT_AT` records the last send. Several PRs merged in a day therefore produce one email, not several. `MODEL_IDS` bypasses the queue.
