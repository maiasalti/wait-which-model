...
- **Sign-up:** `components/SubscribeForm` in the footer, `components/SubscribeBanner` (sticky bar, hidden on `/subscribe`) and the dedicated `/subscribe` page (`components/SubscribePanel`, the link to use in posts) → `app/api/subscribe/route.ts` → Resend `POST /contacts`. Needs `RESEND_API_KEY` + `RESEND_SEGMENT_ID` in Vercel (production only — previews deliberately return 503 "not_configured"). Pure parsing in `lib/subscribe.ts`.
...
- **Cooldown:** a merge within 20 hours of the last email (`NOTIFY_COOLDOWN_HOURS`; the last send time comes from Resend's broadcast history, no GitHub state) sends nothing; the workflow's daily 09:00 UTC scheduled run then announces every model added since that email as one digest. Several PRs merged in a day therefore produce one email. `[skip notify]` backfills are excluded from digests too. `workflow_dispatch` flushes the digest by hand, or announces exactly the ids given in `model_ids` (bypasses the cooldown).
- Subject is `NEW model release: <every added model name, comma-separated>`. Deleting or editing a model never notifies. **Backfills** (adding a model released a while ago) must not email anyone: put `[skip notify]` in the merge commit message and the workflow skips that push. Re-running a workflow run by hand **resends** — don't. Three merges within a few minutes can drop the middle one's email (GitHub queues one pending run per concurrency group).
